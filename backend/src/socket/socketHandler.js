...
const socketHandler = (io) => {
  ...
  io.on("connection", async (socket) => {
    ...
    // We can't easily get friends list here efficiently without query,
    // but we can emit a global 'user-status-change' that clients filter,
    // or iterate active users. For now, let's rely on the client fetching friends list.
    // Better: Emit to all connected users that this user is online
    ...
    socket.on("join-wifi", async ({ ipAddress, ssid, subnet }) => {
      try {
        ...
        if (!clientIP || clientIP === "::1" || clientIP === "127.0.0.1") {
          if (process.env.ALLOW_LOOPBACK === "true") {
            ...
            socket.emit("wifi-group-joined", {
              wifiGroup: null,
              error:
                "Cannot detect network. Please ensure you are on a local network.",
            });
            return;
          }
        }

        const wifiGroup = await wifiService.joinWiFiGroup(
          userId,
          clientIP,
          ssid,
          subnet
        ...
        socket.emit("error", {
          message: "Failed to join WiFi network: " + error.message,
        });
      }
    });
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          ...
          // Send to recipient (all their devices)
